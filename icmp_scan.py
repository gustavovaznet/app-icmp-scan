...
def rotate(addr, file_name, wait, responses):
    ...
    for ip in addr:
        ping(str(ip))
        time.sleep(wait)
    print("All packages have been sent with success!", time.strftime("%X %x %Z"))
    ...
    global SIGNAL
    SIGNAL = False
    #FINAL PING TO TRIGGER A FALSE LISTENING SIGNAL
    ping('127.0.0.1')

    print(len(responses), "All hosts have been found!")
    print("Writing file...")
    hosts = []
...
if __name__ == "__main__":
    ...
    #ADJUST WAIT BASED ON YOUR BANDWIDTH
    wait = 0.0001
    file_name = 'log.txt'
